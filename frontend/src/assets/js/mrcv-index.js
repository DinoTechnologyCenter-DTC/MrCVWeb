  ...
  if (!cvs.length) {
    box.innerHTML = `
      <div class="text-center py-5">
        ...
        <h3 class="h6 mt-3">No CVs yet</h3>
        <p class="text-secondary small">Create your first CV in under 10 minutes — free, no watermark.</p>
        <a href="new-cv.html" class="btn btn-primary btn-sm"><i class="ti ti-plus"></i> Create my CV</a>
      </div>`;
    return;
  }
...
function renderChecklist() {
  ...
  const letters = getLetters();
  const steps = [
    { done: cvs.length > 0, text: 'Create your first CV' },
    { done: letters.length > 0, text: 'Generate a cover letter' },
    { done: cvs.reduce((a, c) => a + (c.downloads || 0), 0) > 0, text: 'Download / share your PDF' },
  ];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('cvList').addEventListener('click', (e) => {
    ...
    if (act === 'del' && !window.confirm('Delete this CV? This cannot be undone.')) return;
